...
async def list_orders(services: Services) -> list[DeliveryOrderRead]:
    rows = await DeliveryOrderRepository(services.session).list_all()
...
async def get_order(order_id: int, services: Services) -> DeliveryOrderRead:
    order = await DeliveryOrderRepository(services.session).get_by_id(order_id)
    if order is None:
...
async def update_order(order_id: int, body: DeliveryOrderCreate, services: Services, user_id: UserDep) -> DeliveryOrderRead:
...
async def delete_order(order_id: int, services: Services, user_id: UserDep) -> None:
