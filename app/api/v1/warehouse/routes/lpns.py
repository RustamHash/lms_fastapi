...
from app.api.v1.warehouse.deps import get_lpn_service
...
async def list_lpns(service: LPNService = Depends(get_lpn_service)) -> list[LPNRead]:
...
async def create_lpn(
    body: LPNCreate,
    user_id: UserDep,
    service: LPNService = Depends(get_lpn_service),
) -> LPNRead:
...
async def get_lpn(
    ...
    lpn = await service.get_by_id(lpn_id)
    if lpn is None:
...
async def update_lpn(
    ...
    lpn = await service.update(lpn_id, user_id=user_id, status=body.status)
    if lpn is None:
        raise NotFoundError("LPN не найдена")
    return LPNRead.model_validate(lpn)
...
async def delete_lpn(
    lpn_id: int,
    user_id: UserDep,
    service: LPNService = Depends(get_lpn_service),
) -> None:
    ok = await service.soft_delete(lpn_id, user_id)
    if not ok:
        raise NotFoundError("LPN не найдена")
